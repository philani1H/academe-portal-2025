...
const ContentManagement = () => {
  ...
  const handleBulkPricingUpload = async (file: File) => {
    ...
    try {
      ...
      const response = await apiFetch<{ message: string; updated: number; created: number; total: number }>(
  ...
  const handleBulkTutorUpload = async (file: File) => {
    ...
    try {
      ...
      await fetchTutors()
      setShowTutorBulkUploadDialog(false)
    } catch (error) {
      console.error('Error uploading tutor data:', error)
      toast({
        title: 'Error',
  ...
  const handleBulkTeamUpload = async (file: File) => {
    ...
    try {
      ...
      setShowTeamBulkUploadDialog(false)
    } catch (error) {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh]">
        ...
        {editingTutor && (
          <div className="grid gap-4 py-4">
            ...
            <div>
              <Label>Subjects/Courses (select from database)</Label>
              <ScrollArea className="h-[160px] w-full rounded-md border p-3 mt-1">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  {subjects.length === 0 && (
                    <div className="text-xs text-gray-500">No subjects available. Add subjects first.</div>
                  )}
                </div>
              </ScrollArea>
            </div>
  ...
  // Bulk Pricing Upload Dialog
  const BulkPricingUploadDialog = () => {
    const handleExportPricing = async () => {
      try {
        const dataStr = JSON.stringify(pricingPlans, null, 2);
        const dataBlob = new Blob([dataStr], { type: 'application/json' });
        const url = URL.createObjectURL(dataBlob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `pricing-plans-export-${new Date().toISOString().split('T')[0]}.json`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        toast({ title: 'Success', description: 'Pricing plans exported successfully' });
      } catch (error) {
        toast({ title: 'Error', description: 'Failed to export pricing plans', variant: 'destructive' });
      }
    };
    ...
    <Dialog open={showBulkUploadDialog} onOpenChange={setShowBulkUploadDialog}>
      <DialogContent className="max-w-3xl max-h-[90vh] w-[95vw] sm:w-full overflow-hidden">
        <DialogHeader>
          <DialogTitle>Bulk Upload Pricing Plans</DialogTitle>
          <DialogDescription>
            Upload a CSV, JSON, or Excel file to update multiple pricing plans at once
          </DialogDescription>
        </DialogHeader>
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => {
                const file = e.target.files?.[0]
                if (file) {
                  handleBulkPricingUpload(file)
                }
              }}
              disabled={uploadingPricing}
            />
            <p className="text-sm text-gray-500">
              {uploadingPricing ? 'Uploading...' : 'Supported formats: CSV, JSON, Excel (.xlsx, .xls)'}
            </p>
            <div className="flex flex-wrap gap-2 mt-2">
              <a 
                href="/templates/pricing-template.csv" 
                download 
              ...
              <a 
                href="/templates/pricing-template.json" 
                download 
                className="text-sm text-blue-600 hover:underline flex items-center gap-1"
              >
                <FileText className="h-4 w-4" />
...
Pro Plan,R599,month,"All Basic|Feature 4|Feature 5","Enterprise Support","bg-purple-500",Zap,true,2`}
              ...
              <p className="text-xs text-gray-600">
                • Use pipe (|) or semicolon (;) to separate array items<br/>
                • Use "true"/"false" for boolean values<br/>
                • Existing plans (matched by name) will be updated<br/>
...
{`[
  {
    ...
    "order": 1
              ...
              <p className="text-xs text-gray-600">
                • Must be a valid JSON array of pricing objects<br/>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowBulkUploadDialog(false)} disabled={uploadingPricing}>
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  // Bulk Tutor Upload Dialog
  const BulkTutorUploadDialog = () => {
    const handleExportTutors = async () => {
      try {
        const dataStr = JSON.stringify(tutors, null, 2);
        const dataBlob = new Blob([dataStr], { type: 'application/json' });
        const url = URL.createObjectURL(dataBlob);
        const link = document.createElement('a');
        link.href = url;
        link.download = `tutors-export-${new Date().toISOString().split('T')[0]}.json`;
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);
        toast({ title: 'Success', description: 'Tutors exported successfully' });
      } catch (error) {
        toast({ title: 'Error', description: 'Failed to export tutors', variant: 'destructive' });
      }
    };

    return (
    <Dialog open={showTutorBulkUploadDialog} onOpenChange={setShowTutorBulkUploadDialog}>
      <DialogContent className="max-w-3xl max-h-[90vh] w-[95vw] sm:w-full overflow-hidden">
        <DialogHeader>
          <DialogTitle>Bulk Upload Tutors</DialogTitle>
          <DialogDescription>
            Upload a CSV, JSON, or Excel file to update multiple tutors at once
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="max-h-[calc(90vh-180px)] pr-4">
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => {
                const file = e.target.files?.[0]
                if (file) {
                  handleBulkTutorUpload(file)
                }
              }}
              disabled={uploadingTutors}
            />
            <p className="text-sm text-gray-500">
              {uploadingTutors ? 'Uploading...' : 'Supported formats: CSV, JSON, Excel (.xlsx, .xls)'}
            </p>
            <div className="flex flex-wrap gap-2 mt-2">
              <a 
                href="/templates/tutors-template.csv" 
                download 
                className="text-sm text-blue-600 hover:underline flex items-center gap-1"
              >
                <FileText className="h-4 w-4" />
              ...
              <span className="text-gray-300">|</span>
              <a 
                href="/templates/tutors-template.json" 
                download 
                className="text-sm text-blue-600 hover:underline flex items-center gap-1"
              >
                <FileText className="h-4 w-4" />
...
John Doe,"Math|Physics|Chemistry","https://example.com/image.jpg","Jane Doe","+27123456789","john@example.com","Experienced tutor",1`}
              ...
              <p className="text-xs text-gray-600">
                • Required: name, subjects<br/>
                • Use pipe (|) or semicolon (;) to separate subjects<br/>
                • Existing tutors (matched by name) will be updated<br/>
                • Excel: Save as CSV before uploading
              </p>
            </div>
...
{`[
  {
    ...
    "order": 1
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowTutorBulkUploadDialog(false)} disabled={uploadingTutors}>
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  // Bulk Team Upload Dialog
  const BulkTeamUploadDialog = () => (
    <Dialog open={showTeamBulkUploadDialog} onOpenChange={setShowTeamBulkUploadDialog}>
      <DialogContent className="max-w-3xl max-h-[90vh] w-[95vw] sm:w-full overflow-hidden">
        <DialogHeader>
          <DialogTitle>Bulk Upload Team Members</DialogTitle>
          <DialogDescription>
            Upload a CSV or JSON file to update multiple team members at once
          </DialogDescription>
        </DialogHeader>
        <ScrollArea className="max-h-[calc(90vh-180px)] pr-4">
        <div className="space-y-4 py-4">
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(e) => {
                const file = e.target.files?.[0]
                if (file) {
                  handleBulkTeamUpload(file)
                }
              }}
              disabled={uploadingTeam}
            />
            <p className="text-sm text-gray-500">
              {uploadingTeam ? 'Uploading...' : 'Supported formats: CSV, JSON'}
            </p>
          </div>
...
{`[
  {
    ...
    "order": 1
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setShowTeamBulkUploadDialog(false)} disabled={uploadingTeam}>
            Close
          </Button>
        </DialogFooter>
      </DialogContent>
  ...
  return (
    <div className="container mx-auto p-6">
                    ...
                    <CardContent>
                      <div className="flex justify-between items-center mb-4">
                        ...
                        <div className="flex gap-2">
                          <Button 
                            variant="outline"
                            onClick={() => setShowTeamBulkUploadDialog(true)}
                          >
                            <Upload className="h-4 w-4 mr-2" />
                            Bulk Upload
                          </Button>
              ...
              <CardContent>
                <div className="flex justify-between items-center mb-4">
                  ...
                  <div className="flex gap-2">
                    <Button 
                      variant="outline"
                      onClick={() => setShowTutorBulkUploadDialog(true)}
                    >
                      <Upload className="h-4 w-4 mr-2" />
                      Bulk Upload
                    </Button>
                    ...
                    <Button
                      ...
                      onClick={async () => {
                        try {
                          const res = await apiFetch<any>('/api/admin/tutors/normalize-emails', {
                            method: 'POST'
                          })
                          await fetchTutors()
                          toast({
                            title: 'Emails Updated',
                            description: `Updated ${res?.updated ?? 0} tutors to @${res?.domain || 'excellenceakademie.co.za'}`,
                          })
                        } catch (error) {
                          console.error('Normalize emails error:', error)
                          toast({
                            title: 'Error',
                            description: 'Failed to update tutor emails',
                            variant: 'destructive',
                          })
                        }
                      }}
      ...
      <BulkPricingUploadDialog />
      <BulkTutorUploadDialog />
      <BulkTeamUploadDialog />
