...
app.get('/api/users/:id', async (req, res) => {
    try {
        const user = await (0, db_1.executeQuery)('SELECT * FROM users WHERE id = ?', [req.params.id]);
        if (user) {
            res.json(user);
        }
        else {
...
app.get('/api/courses', async (req, res) => {
    try {
        const courses = await (0, db_1.executeQuery)('SELECT * FROM courses');
        res.json(courses);
    }
    catch (error) {
...
app.get('/api/courses/:id', async (req, res) => {
    try {
        const course = await (0, db_1.executeQuery)('SELECT * FROM courses WHERE id = ?', [req.params.id]);
        if (course) {
            res.json(course);
        }
        else {
...
app.get('/api/admin/content/:type', async (req, res) => {
    try {
        ...
        switch (contentType) {
            case 'tutors':
                content = await (0, db_1.executeQuery)('SELECT * FROM tutors');
                break;
            case 'team-members':
                content = await (0, db_1.executeQuery)('SELECT * FROM team_members WHERE is_active = 1 ORDER BY [order] ASC, created_at DESC');
                break;
            case 'about-us':
                content = await (0, db_1.executeQuery)('SELECT * FROM about_us_content WHERE is_active = 1 ORDER BY updated_at DESC');
                break;
            case 'hero':
                content = await (0, db_1.executeQuery)('SELECT * FROM hero_content LIMIT 1');
                break;
            case 'features':
                content = await (0, db_1.executeQuery)('SELECT * FROM features');
                break;
            case 'announcements':
                content = await (0, db_1.executeQuery)('SELECT * FROM announcements ORDER BY pinned DESC, created_at DESC');
                break;
            case 'testimonials':
                content = await (0, db_1.executeQuery)('SELECT * FROM testimonials');
                break;
            case 'pricing':
                content = await (0, db_1.executeQuery)('SELECT * FROM pricing_plans');
                break;
            case 'events':
                content = await (0, db_1.executeQuery)('SELECT * FROM events ORDER BY date ASC');
                break;
            case 'footer':
                content = await (0, db_1.executeQuery)('SELECT * FROM footer_content LIMIT 1');
                break;
            case 'subjects':
                content = await (0, db_1.executeQuery)('SELECT * FROM subjects');
                break;
            case 'navigation':
                content = await (0, db_1.executeQuery)("SELECT path, label, type FROM navigation_items WHERE is_active = 1 ORDER BY [order] ASC, created_at ASC");
                break;
            case 'exam-rewrite':
                content = await (0, db_1.executeQuery)('SELECT * FROM exam_rewrite_content WHERE is_active = 1 ORDER BY updated_at DESC LIMIT 1');
                break;
            case 'university-application':
                content = await (0, db_1.executeQuery)('SELECT * FROM university_application_content WHERE is_active = 1 ORDER BY updated_at DESC LIMIT 1');
                break;
            default:
...
async function initializeDatabase() {
    try {
        const db = await (0, db_1.getConnection)();
        const files = [
            path_1.default.join(baseDir, 'database', 'schema.sql'),
            path_1.default.join(baseDir, 'database', 'content-schema.sql')
        ];
        for (const file of files) {
            try {
                const sql = await promises_1.default.readFile(file, 'utf8');
...
async function seedDatabaseIfEmpty() {
    try {
        ...
        for (const s of singletons) {
            try {
                const row = await db.get(`SELECT COUNT(*) as c FROM ${s.table}`);
                if ((row?.c ?? 0) === 0) {
        ...
        for (const c of collections) {
            try {
                const row = await db.get(`SELECT COUNT(*) as c FROM ${c.table}`);
                if ((row?.c ?? 0) === 0) {
...
initializeDatabase().then(() => seedDatabaseIfEmpty()).then(() => {
    ...
    server.on('error', (error) => {
        if (error.code === 'EADDRINUSE') {
            ...
            app.listen(fallback, '0.0.0.0', () => console.log(`Server running at http://localhost:${fallback}`));
        }
        else {
